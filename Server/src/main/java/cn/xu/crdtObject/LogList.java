...
public class LogList implements CrdtObject{
    ...
    private HashMap<String, Msg> msgMap;

    public LogList() {
        this(0);
    }

    public LogList(int waitMS) {
        this.waitMS = waitMS;
    ...
    public void msgIn(Msg msg) {
        ...
        // 消息记录
        msgList.add(msg);
        msgMap.put(((MultiEdgeClock)msg.getClock()).unique(), msg);
        // 消息转发
        if (waitMS == 0) {
    ...
    private void dealMsg(Msg msg) {
        ...
        if (op != null) {
            if (op.getOpType() == OpType.REMOVE) {
                MultiEdgeClock msgClock = MultiEdgeClock.deSerialized(op.getParams()[0]);
                Msg tempMsg = msgMap.get(msgClock.unique());
                if (tempMsg == null) {
                    System.out.println(msgMap);
                    System.out.println(op.getParams()[0]);
                    return;
                }
                tempMsg.invalid();
            }
